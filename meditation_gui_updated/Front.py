...
from paths import (FRONT_CAM_INDEX, fullscreen_window, project_path, upright,
                   upright_size)
...
ref_image_path = project_path("Screenshot from 2025-05-07 10-45-22.png")
...
def _reference_pose():
    """One Pose model, reused across all nine reference stills.

    Building a fresh model_complexity=2 instance per image cost ~0.9s of stage
    startup — visible as the black "Session in progress" screen between stages.
    static_image_mode=True carries no state between images, so reuse gives
    identical results.
    """
    global _REF_POSE
...
# Function to extract angles from a set of pose landmarks
...
def referenceAngles(image_paths):
    """Angles and pixel landmarks for each reference still.

    The inference behind this used to run on every launch and cost ~0.95s of
    stage startup, which the visitor saw as part of the black gap between
    stages; posecache keeps the landmarks on disk instead. static_image_mode
    Pose treats each image independently, so only stills that actually changed
    are re-inferred.
    """
    results = []
    for entry in posecache.landmarks_for(image_paths, 'pose_static_mc2',
                                         _reference_pose):
        if not entry['landmarks']:
            results.append((None, None))
            continue
        width, height = entry['width'], entry['height']
        landmarks = [(int(x * width), int(y * height), z * width)
                     for x, y, z in entry['landmarks']]
        results.append((_angles(landmarks), landmarks))
    return results


def analyzeVideo(reference_angles1, reference_landmarks1,reference_angles2, reference_landmarks2,reference_angles3, reference_landmarks3,reference_angles4, reference_landmarks4,reference_angles5, reference_landmarks5,reference_angles6, reference_landmarks6,reference_angles7, reference_landmarks7,reference_angles8, reference_landmarks8,reference_angles9, reference_landmarks9):
    with mp_pose.Pose(min_detection_confidence=0.35, model_complexity=2) as pose:
        ...
        # Size must follow upright()'s rotation — see check_similarity4.py.
        out = cv2.VideoWriter(str(project_path('visual1_video.avi')), fourcc, 20.0,
                              upright_size(640, 480))
        start_time = time.time()
        # Borderless full screen, no Qt toolbar (see paths.fullscreen_window).
        fullscreen_window('Front')
        while cap.isOpened():
            elapsed_time = time.time() - start_time
            #print(f"Elapsed Time: {elapsed_time:.2f} seconds")  # Debugging print for elapsed time
    
            if elapsed_time > 15:  # 5 minutes (300 seconds)
               #print("5 minutes reached. Exiting...")
               break
    
            success, frame = cap.read()  # Webcam feed (right side)
            frame = upright(frame)       # front camera is mounted 90° rotated
            #success1, frame2 = cap2.read()  # Existing video feed (left side)
            # Get the elapsed time to stop after 5 minutes
            ...
            combined = np.hstack((resized_ref, resized_frame))
...
reference_image_paths = [
    project_path("Screenshot from 2024-10-30 11-45-14.png"),
    project_path("Screenshot 2024-11-08 122356.png"),
    project_path("Screenshot 2024-11-08 122257.png"),
    project_path("Screenshot 2024-11-08 122008.png"),
    project_path("Screenshot 2024-11-08 121728.png"),
    project_path("Screenshot 2024-11-08 121652.png"),
    project_path("Screenshot 2024-11-08 121631.png"),
    project_path("Screenshot 2024-11-08 121523.png"),
    project_path("Screenshot 2024-11-08 121407.png"),
]

#video_path = 'BK_1_FIVE_MINS(1).mp4'

# Get the reference angles
((reference_angles1, reference_landmarks1),
 (reference_angles2, reference_landmarks2),
 (reference_angles3, reference_landmarks3),
 (reference_angles4, reference_landmarks4),
 (reference_angles5, reference_landmarks5),
 (reference_angles6, reference_landmarks6),
 (reference_angles7, reference_landmarks7),
 (reference_angles8, reference_landmarks8),
 (reference_angles9, reference_landmarks9)) = referenceAngles(reference_image_paths)
if reference_angles1:
    print("Reference angles:", reference_angles1)
    analyzeVideo(reference_angles1, reference_landmarks1,reference_angles2, reference_landmarks2,reference_angles3, reference_landmarks3,reference_angles4, reference_landmarks4,reference_angles5, reference_landmarks5,reference_angles6, reference_landmarks6,reference_angles7, reference_landmarks7,reference_angles8, reference_landmarks8,reference_angles9, reference_landmarks9)
else:
