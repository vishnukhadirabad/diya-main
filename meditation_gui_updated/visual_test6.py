...
from paths import GAZE_CAM_INDEX, project_path
...
output_path = project_path("Gaze_output.avi")
...
video_path = project_path("BK_1_FIVE_MINS.mp4")
# Initialize Webcam
cap = cv2.VideoCapture(GAZE_CAM_INDEX)
...
#     wait_time = int(1000 / fps) if fps > 0 else 25  # Fallback to 25ms if FPS is 0
...
total = 0
look = 0
# score is only assigned once a face has been detected, but the timeout check
# below reads it on every iteration — with no detection yet that raised
# NameError and killed the capture at the end of the recording window. Start at
# 0, which is the value that check means by "could not reach position".
score = 0
...
while cap.isOpened():
       ...
       continue
    ...
    elif (((time.time() - start_time) < recording_time)&(time.time()>start_time)):
        ...
        if results.multi_face_landmarks:
            ...
            if result_hands.multi_hand_landmarks:
                ...
                ng = (int(((left_pupil.x * frame_width-r*cvl*shl)+(right_pupil.x * frame_width-r*cvr*shr))/2), int(((left_pupil.y * frame_height-r*svl)+(left_pupil.y * frame_height-r*svr))/2))
                ...
                print(score,"%")
        print("saving frame...")
        #cv2.imshow('Pupils', rgb_frame)
        out.write(frame)
    
    
    elif (((time.time() - start_time) > recording_time)|(((time.time() - start_time) > check_time)&(score == 0))):#could not reach position or over recording time
        # print("elif")
        break
