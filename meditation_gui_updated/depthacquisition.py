...
from paths import project_path, realsense_serial
...
# Set the correct device serial number. Serial was hardcoded to a camera that
# is no longer attached (see paths.realsense_serial).
_serial = realsense_serial()
if _serial:
    config.enable_device(_serial)
...
def load_reference_images(reference_folder):
    ...
    for filename in os.listdir(reference_folder):
        if filename.endswith('.jpg') or filename.endswith('.png'):
            img_path = reference_folder / filename
...
reference_folder = project_path("ref")  # Folder containing reference images
